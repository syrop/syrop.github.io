...
This is an article about the [MVVM design pattern][mvvm]. It previous articles I erroneously callet it the [MVP pattern][mvp] beacause I misunderstood the role of presenter in the latter.
...
You can just *invoke* the `LiveData` with a `LifecycleOwner` and an `Observer` and it will *observe* the `LiveData` for you. To remind you a syntax that calls the above `operator` function:
