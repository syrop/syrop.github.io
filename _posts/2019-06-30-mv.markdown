...
When `LiveData` is extended manually by overriding the functions [`onActive()`][on-active] and [`onInactive()`][on-inactive], in the way I explained in [another article][extending-article] in this blog, the `Channel` is closed immediately, so the developer may write tests without taking timeout into considerations. Doing so is outside of the scope of the present article.
...
[testing-article]: https://syrop.github.io/jekyll/update/2019/06/20/viewmodel-mockito.html
